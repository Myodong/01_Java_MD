...
public class UserView {
	...
	private User user = null; //회원 가비 정보를 담고 있는 User 객체 참조
	
	// 로그이니 한 User 객체의 주소를 참조할 변수
	...
	public void displayMenu() {
		...
		do {
			...
			input = sc.nextInt(); // 입력 버퍼 개행문자 제거
			sc.nextLine();
	...
	public void signUp() {
		...
		// - 필드에 존재하는 user 참조 변수에 반환된 User 객체의 주소를 저장 한다.
	...
	public void login() {
		...
		if (loginUser != null) {
			...
			int result = service.login(id, pw, loginUser); // 로그인 서비스 호출
			// -1 , 0 , 0
			
			if (result == -1) {
				System.out.println("회원 가입 후 시도해 주세요");
			}else if (result == 0) {
				System.out.println("아이디 또는 비밀번호가 일치하지 않습니다.");
			}else {
				System.out.println("로그인 성공");
				loginUser = user; // user 변수가 참조하는 객체의 주솔를 loginUser에 대입(얕은복사)
			}	
		}
	}
	...
	public void printUser() {
		...
		if (loginUser != null) {// 로그인 일때
			
			// loginUser 가 참조하는 객체에서 getter를 이용해 필드 값을 얻어와 출력
