...
// model :비지니스 로직 (데이터 가공, 관리, DB 연결 등)
 // service : 기능 제공용 클래스
public class StudentManagementService {

	// Student 참조변수 5칸 자리 객체 배열 생성
	 ...
	 */
	public int addStudent(int grade, int classRoom, int number, String naem) {
		...
		stdArr[idx] = new Student(grade, classRoom, number, naem);
		return 1;
	}
	 ...
	 * @param math
	 * @return
	 *  -1 : idx 가 stdArr배열의 범위를 초과한 경우
	 *   0 : stdArr[idx] 인덱스가 null인 경우
	 *   1 : 정상적으로 수정이 된 경우
	 */
	public int updateStudent(int idx, int kor, int eng, int math) {
		if (idx > stdArr.length) { // idx 가 stdArr배열의 범위를 초과한 경우
			return -1;
		} else {
			if (stdArr[idx] == null) { //stdArr[idx] 인덱스가 null인 경우
				return 0;
			} else { //정상적으로 수정이 된 경우
	...
	public Student[] selectName(String name) {
		...
		if (resultIdx==0) { // 검색이 아무도 되지 않은 경웅
			return null;
		}else { // 결과 있음
			return resultArr;
		}
