...
/*#############################################################################*/
	public void ex2() {
		...
		arr = new int[4];
		//heap 영역에 int 칸짜리 배열을 항당(생성)하고
		//할당된 배열의 시작주소를 arr에 대입
		
		//배열길이 (크기)	:4
		System.out.println("배열 길이 : "+arr.length);
		//arr.length : arr이 참조하고 있는 배열의 기리
	
		System.out.println("컴파일러확인");
		...
		//stack 영역은 컴파일러가 알아서 초기허허ㅘ)
		//heap 영역은 컴파일러가 알아서 초기화
		int a;
...
/*#############################################################################*/
	...
	// 점심 메뉴 뽑기 프로그램
		...
		String[] menuArr = {"김밥", "라면", "햄버거", "짜장면", "편의점"};
...
/*#############################################################################*/
	...
	public void ex7() {
		...
		// 인원 수를 입력 받아 그 크기만큼의 정수배열을 선언 및 할당
		// 인원 수 만큼 점수를 입력 받아
		//합계, 평균, 최고점, 최저점
		...
		// breakpoint를 지정한 라인 이전에 멈춘다.(debug mode)
		//for문에 breakpoint를 지정하면 증감식이 해석되기 전에 멈춘다.(debug mode)
		for (int i = 1; i < scoreArr.length; i++) {
			
			if (scoreArr[i]>max) {//기존 최고점 보다 scoreArr[i] 값이 큰 경우
				max= scoreArr[i];
			} 
...
/*#############################################################################*/	
	public void ex8() {
		...
		//배열 내 데이터 검색기능
		//입력받은 정수가 배열에 존재하면 몇 번 인텍스에 존재하는지 출력
		//단, 없다면 "존재하지 않습니다." 출력
		...
		boolean flag=true;
		//for문이 종료된 후에도 true : 존재하지 않는다.
		//for문이 종료된 후에도 false : 존재한다.		
		...
		for (int i = 0; i < arr.length; i++) {
			...
			if(arr[i] == search) {// arr[i] 값이 입력 받은 값과 같다면
				...
				break;//중복데이터가 없으면 추가 검색을 하지 않아도 됨
...
/*#############################################################################*/	
	...
	public void createlottoNumber() {
		...
		for (int i = 0; i < lotto.length; i++) {
			...
			//현재 인덱스 이전의 요소를 순차 접근
			for (int x = 0; x < i; x++) {
				
				if (lotto[x] == ran) {
					i--;// i값을 인위적으로 감소시켜
						// 바깥쪽 for의 증감식 (i++)이 실행 되었을때
						// i 값이 현재 값을 유지하도록 만든다.
						// (인덱스를 왼쪽 1칸 +오른쪽 1칸 ==제자리
...
public void ex9() {
	//얕은 복사
	// - 참조하는 주소만을 복사하여
	// 서로다른 참조 변수가 하나의 배열 (또는 객체)를 참조하게 하는 복사방법
	
	//특징 : 하나의 배열을 참조하기 때문에 값을 공유하게 된다.
	
	int[] arr= {99, 70 , 80, 50 , 40 };
	
	//arr 변수에 저장된 배열의 시작 주소를 copyArr에 대입(얕은복사) 
	int[] copyArr = arr;
	...
	//복사본의 값을 변경 -> 원본도 값이 변함 == 값을 공유(얕은복사 특징)
	copyArr[2] = 10000;
...
/*#############################################################################*/	
	...
	public void ex11() {
		...
		//arr1==null -> 결과false == arr1 참조 변수가 무언가를 참조하고 있다.
