...
public class Example1 {
	...
	public void ex1() {
		...
		c2.setSeat(1);
		c2.setFuel("휘발유");
//      c2.setweight(1.5); // Error
		// The method setWeight(double) is undefind for the type Car
...
//		System.out.println(c1.toString());
		...
		// 동적 바인딩을 이용해서 자식 toSrting() 호출

		
		public void ex3() {
			
			// 다운 캐스팅
			//  - 업 캐스팅이 적용 상황에서 (부모 참조 변수 = 자식객체)
			// 	  부모 참조변수를 자식 타입으로 바꿔(강제 형변환, 얕은 복사)
			//	  자식 객체를 온전하게 참조할 수 있게 만듬.
...
//			((Truck)c3).loading();
		...
		public void ex4() {
			// instanceof연산자 사용 시 검사 순서에 대한 문제점
			...
			if (c instanceof Car) {
				...
				System.out.println("부모 Car 타입 입니다.");
			}else {
				((Spark)c).dc();
			}
		...
		public void check(Car c) {
			// 전달 받은 c의 타입을 검사 해서 고유한 메서드를 호출
			
			if (c instanceof Spark) {// 참조하는 객체가 Spark인 경우
				((Spark)c).dc();
			}else if (c instanceof Truck) {// 참조하는 객체가 Truck 인경우
				((Truck)c).loading();
			}else { //참조하는 객체가 Car인경우
