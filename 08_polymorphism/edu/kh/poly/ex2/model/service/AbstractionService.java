...
import edu.kh.poly.ex2.model.vo.KHDCalculator;
...
//		f1.breath();
		...
		// -> 다형성의 업 캐스팅
		Animal a1 = new Fish();
		Animal a2 = new Person(); // 업 캐스팅 확인

		// 추상 메서드 였던 breath() 호출 가능 여부 확인
		a1.breath();
		a2.breath();
		// void edu.kh.poly.ex2.model.vo.Animal.breath()
		// 실행 전에는 Animal의 breath()와 연결 된 것으로 보임(정적 바인딩)
		// 실행 중 오버라이딩한 자식의 breath()로 연결됨(동전 바인딩)
		...
		// 가지고 있으므로 arr[i].breath(); 코드 작성 가능
...
//		KH k1 = new KH();//Cannot instantiate the type KH
	...
	public void ex3() {
		...
		Calculator cal = new KHDCalculator();
